...
restore_shadow() {
  [[ -e /etc/shadow.magikimage_bak ]] || return
  mv /etc/shado{w.magikimage_bak,w}
}
...
rescue_gen_password_hash() {
  ...
  cp /etc/shado{w,w.magikimage_bak}
...
check_rescue_password_hashing_algo_supported_by_installed_os() {
...

  {
    ...
    echo -e "You need to fix this mismatch before you can rerun magikimage.\n"

...
while getopts "han:b:r:l:i:p:v:d:f:c:R:s:z:x:gkK:t:u:G:" OPTION ; do
  case $OPTION in
    ...
    h)
      echo
      echo "usage:  magikimage [options]"
      echo
      echo "  without any options, magikimage starts in interactive mode."
    ...
    c)
      ...
      if grep -q PASSWD /autosetup ; then
        ...
        echo -e "${YELLOW}(or edit /autosetup manually and run magikimage without params)${NOCOL}\n"
        echo -en "PASSWORD:  "
        read -s imagepasswd

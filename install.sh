...
if [ -n "$REUSE_FSTAB" ] ; then
  # copy existing fstab to magikimage path
...
#
# Save magikimage configuration and debug file on the new system
#
(
  echo "#"
  ...
  echo "# system via magikimage script. Comments have been removed."
  echo "#"
  echo "# More information about the magikimage script and"
  echo "# automatic installations can be found in our wiki:"
  echo "#"
  echo "#"
  echo
  cat $FOLD/install.conf | grep -v "^#" | grep -v "^$"
) > $FOLD/hdd/magikimage.conf
cat /root/debug.txt > $FOLD/hdd/magikimage.debug
chmod 640 $FOLD/hdd/magikimage.conf
chmod 640 $FOLD/hdd/magikimage.debug

...
create_config() {
  if [ "$1" ]; then
    ...
    {
      echo "## ======================================================"
      echo "##  $COMPANY - magikimage - standard config"
      echo "## ======================================================"
      echo ""
    } > "$CNF"
...
if [ -n "$1" ]; then
  ...
  HASROOT=0
  # count disks again, for setting COUNT_DRIVES correct after restarting magikimage
  getdrives
...
validate_vars() {
 if [ "$1" ]; then
  ...
  if [[ "$IAM" == centos ]] && (((IMG_VERSION >= 60 && IMG_VERSION < 70)) || ((IMG_VERSION == 610))); then
    graph_error "ERROR: CentOS 6 is EOL since Nov 2020 and magikimage does not support CentOS 6 anymore"
    return 1
  fi
  ...
  if [ "$1" -a "$2" -a "$3" ]; then
    case $2 in
      ...
      http)
        mkdir "$FOLD/keys/" 2>&1
        cd "$FOLD/keys/"
        # no exitcode, because if not found vpsuniverse-pubkey will be used
...
generate_resolvconf() {
  ...
  echo -e "### $COMPANY magikimage" > $DNSRESOLVERFILE
...
set_hostname() {
  ...
  if [[ -n "$hostname" && ( -n "$ip4" || -n "$ip6" ) ]]; then
    ...
    echo "### $COMPANY magikimage" > $hostsfile
...
exit_function() {
  ...
  echo "fails again, please contact our support via VPSUniverse Robot, providing"

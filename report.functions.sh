...
report_install() {
  ...
  curl --data-urlencode "config@$FOLD/install.conf.filtered" \
    ...
    "https://$statsserver/api/v1/magikimage/installations" > "$FOLD/install_report.response"
  ...
  curl -H 'Content-Type: text/plain' \
    ...
    -D "$FOLD/install_report.headers" \
    -X POST \
    "https://$statsserver/api/v1/magikimage/installations/$image_id/logs" > /dev/null
